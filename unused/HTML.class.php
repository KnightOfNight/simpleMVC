...
class HTML {
	static function includeCSS ($css) {
		global $config;

		return (sprintf ("<link rel=\"stylesheet\" type=\"text/css\" media=\"screen\" href=\"%s/css/%s\" />\n", $config->getVal("application.base_url"), $css));
	}

	static function includeJS ($js) {
		global $config;

		return (sprintf ("<script type=\"text/javascript\" src=\"%s/js/%s\"></script>\n", $config->getVal("application.base_url"), $js));
	}
